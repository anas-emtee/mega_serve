    ...
    <div class="page-wrapper">
        ...
        <div class="page-container2">
            ...
            <?php 
                ...
                $con = Dbcon();                                                 
                $dr = mysqli_query($con, "SELECT * FROM `merchandise_docs` WHERE `req_for`='sijil_a'");
                while($doc = mysqli_fetch_array($dr)){
                    $dref = $doc["id"];
                    $doc_tit = $doc["id"]."_".$doc["document_title"]."_".$sijil["id"]."_".$tit["id"];
                    $ur = mysqli_query($con, "SELECT * FROM `sijil_uploads` WHERE `document`=$dref AND `sc_id`=$item AND `sc_type`='Sijil A' AND `status`='valid'") or die(mysqli_error($con));
                    ...
                    <div class="modal fade" id="mediumModal<?= $dref ?>" tabindex="-1" role="dialog" aria-labelledby="mediumModalLabel" aria-hidden="true">
                        <div class="modal-dialog modal-lg" role="document">
                            <div class="modal-content">
                                ...
                                ?>
                                    <div class="modal-body">
                                        <img src="<?= $doc_src ?>" style="width: 100%; height: 50vh;" class="img-responsive" alt="<?= $doc_src ?>" />
                                    </div>
                                    <div class="modal-footer">
                                        <a href='<?= $upload["document_src"] ?>' class="btn btn-info" download="" target="_blank" > Download <i class="fa fa-download"></i></a>
                                ...
                                <?php }else{ ?>
                                    <form action="main_action.php" method="post" enctype="multipart/form-data">
                                        <div class="modal-body">
                                            ...
                                            <input type="hiddenx" name="sc_id" value="<?= $sijil["id"] ?>" class="form-control">
                                            <input type="hiddenx" name="sc_type" value="Sijil A" class="form-control">
                                            <div class="form-group">
                                        ...
                                        <div class="modal-footer">
                                            <button type="button" class="btn btn-secondary" data-dismiss="modal">Cancel</button>
                                            <button type="submit" name="upload_sijila_document" class="btn btn-primary">Confirm</button>
                                        </div>
                                    </form>
                                <?php } ?>
                            </div>
                                ...
                                <table class="table table-hover table-striped table-align-middle mb-0">
                                  ...
                                  <tbody>
                                    <?php 
                                        ...
                                        while($doc = mysqli_fetch_array($dr)){
                                            $dref = $doc["id"];
                                            $ur = mysqli_query($con, "SELECT * FROM `sijil_uploads` WHERE `document`=$dref  AND `sc_id`=$item AND `sc_type`='Sijil A' AND `status`='valid'");
                                            $up_count = mysqli_num_rows($ur);
                                            if($up_count > 0){
                                                $upload = mysqli_fetch_array($ur);
                                                $allow_act_class = "";
